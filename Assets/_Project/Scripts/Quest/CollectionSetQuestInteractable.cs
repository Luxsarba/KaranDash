...
public class CollectionSetQuestInteractable : MonoBehaviour, IPlayerInteractable
{
    [Header("Quest")]
    [SerializeField] private CollectionSetData requiredSet;
    ...
    private bool HasRequiredSet(PlayerInventory inventory)
    {
        if (inventory == null || requiredSet == null)
            return false;

        bool[] states = inventory.GetCollectionPieceStates(requiredSet);
